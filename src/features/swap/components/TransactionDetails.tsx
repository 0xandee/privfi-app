      ...
      >
        ...
        <div className="flex justify-between items-center">
          <div className="flex items-center">
            <AnimatedNumber
              value={rateWithUsd ? rateWithUsd.split(' (')[0] : rate}
              className="transaction-detail-value text-xs"
            />
            {rateWithUsd && rateWithUsd.includes(' (') && (
              <span className="text-muted-foreground ml-1 text-xs">
                ≈ {rateWithUsd.split(' (')[1].replace(')', '')}
              </span>
            )}
          </div>
          <motion.div
          ...
          >
            <span
              className="flex items-center gap-2 h-8 text-xs text-muted-foreground cursor-pointer hover:opacity-70 transition-opacity duration-200"
              onClick={handleToggleExpand}
            >
              <span className="text-transaction-detail">
                  ...
                  <div className="flex gap-2">
                    {presetSlippages.map((preset) => (
                      <motion.button
                        ...
                        className={`percentage-button -mt-1.5 ${slippage === preset ? 'bg-white text-black hover:bg-white' : ''
                          } ${transitions.default}`}
